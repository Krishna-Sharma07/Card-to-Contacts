// ISO 3166-1 alpha-2 region code -> E.164 calling code.
export const COUNTRY_CALLING_CODES: Record<string, string> = {
  ...
  SD: '+249', SE: '+46', SG: '+65', SH: '+290', SI: '+386', SK: '+421', SL: '+232',
  ...
  UZ: '+998', VA: '+379', VC: '+1', VE: '+58', VG: '+1', VI: '+1', VN: '+84',
  VU: '+678', WS: '+685', YE: '+967', YT: '+262', ZA: '+27', ZM: '+260', ZW: '+263',
};

export function callingCodeForRegion(regionCode?: string): string | undefined {
  if (!regionCode) {
    return undefined;
  }
  return COUNTRY_CALLING_CODES[regionCode.toUpperCase()];
}

const KNOWN_CALLING_CODES = new Set(Object.values(COUNTRY_CALLING_CODES));

// Calling codes are a prefix-free set (no valid code is itself the start of
// another valid code), so trying the longest digit run first and shrinking
// until it matches a real code correctly separates, e.g., "9179776..." into
// India's "+91" rather than greedily grabbing an unassigned "+917".
export function longestKnownCallingCodePrefix(digitsAfterPlus: string): string | undefined {
  for (let length = 3; length >= 1; length -= 1) {
    const candidate = `+${digitsAfterPlus.slice(0, length)}`;
    if (KNOWN_CALLING_CODES.has(candidate)) {
      return candidate;
    }
  }
  return undefined;
